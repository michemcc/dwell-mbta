...
On escalators, stand on the right, walk on the left. This is enforced by social pressure and occasional pointed staring. At busy stations like Downtown Crossing and Park Street there are audible announcements reminding people of this. The rule is taken seriously.`,
    ...
    title:    'The Complete MBTA Commuter Rail Guide for 2026',
...
function GuidesIndex({ onSelect }) {
  return (
    <div className="anim-fade-in">
      ...
      {/* Bento grid: hero card full-width, rest in 2-col grid */}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>

        {/* Hero — newest article, spans full width */}
        {ARTICLES[0] && (() => { const a = ARTICLES[0]; return (
          <button key={a.id} onClick={() => onSelect(a)}
            className="anim-fade-up"
            style={{
              animationDelay: '0s',
              display: 'flex', flexDirection: 'column', gap: 14,
              padding: '28px 28px 24px', textAlign: 'left', width: '100%',
              background: 'linear-gradient(135deg, rgba(245,206,62,0.07) 0%, var(--bg-3) 60%)',
            ...
            onMouseEnter={e => {
              e.currentTarget.style.background = 'linear-gradient(135deg, rgba(245,206,62,0.13) 0%, var(--bg-3) 60%)'
              e.currentTarget.style.boxShadow = '0 4px 32px rgba(245,206,62,0.12)'
              e.currentTarget.style.transform = 'translateY(-2px)'
            }}
            onMouseLeave={e => {
              e.currentTarget.style.background = 'linear-gradient(135deg, rgba(245,206,62,0.07) 0%, var(--bg-3) 60%)'
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
              ...
              <span style={{ fontFamily: 'var(--mono)', fontSize: 9, color: 'var(--accent)', letterSpacing: '0.18em', border: '1px solid var(--accent-dim)', padding: '3px 8px', borderRadius: 2, flexShrink: 0 }}>LATEST</span>
            </div>
            <h2 style={{
              fontFamily: 'var(--display)', fontSize: 'clamp(16px, 3vw, 20px)', fontWeight: 800,
              letterSpacing: '0.02em', color: 'var(--text)', lineHeight: 1.25,
            }}>{a.title}</h2>
            <p style={{ fontFamily: 'var(--sans)', fontSize: 14, color: 'var(--text-muted)', lineHeight: 1.6, margin: 0 }}>
              {a.intro.slice(0, 160)}{a.intro.length > 160 ? '…' : ''}
            </p>
            <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
              <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--text-dim)', letterSpacing: '0.06em' }}>
                {AUTHOR.name} · {a.dateDisplay} · {a.readMins} min read
              </span>
              <span style={{ marginLeft: 'auto', fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--accent)', letterSpacing: '0.1em', fontWeight: 600 }}>READ →</span>
            </div>
          </button>
        )})()}

        {/* 2-column bento grid for remaining articles */}
          ...
          {ARTICLES.slice(1).map((a, i) => (
            ...
            >
              <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                {a.tags.slice(0, 2).map(t => <TagBadge key={t} label={t} />)}
              </div>
              <h2 style={{
                fontFamily: 'var(--display)', fontSize: 13, fontWeight: 700,
                letterSpacing: '0.02em', color: 'var(--text)', lineHeight: 1.35, flex: 1,
              }}>{a.title}</h2>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 'auto' }}>
                <span style={{ fontFamily: 'var(--mono)', fontSize: 9, color: 'var(--text-dim)', letterSpacing: '0.06em' }}>
                  {a.dateDisplay} · {a.readMins} min
                </span>
                <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--accent)' }}>→</span>
              </div>
            </button>
          ))}
        </div>
